...
function getParentElement(node: ChildNode): Element | null {
  return isElementNode(node) ? node.parentElement : (node.parentNode as Element);
}
 ...
 */
export async function generateXPathsForElement(element: ChildNode): Promise<string[]> {
