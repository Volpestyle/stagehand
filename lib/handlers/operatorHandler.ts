...
import { OperatorResponse, operatorResponseSchema, OperatorSummary, operatorSummarySchema } from '@/types/operator';
...
export class StagehandOperatorHandler {
    ...
    while (!completed && currentStep < maxSteps) {
      ...
      if (result.method === 'extract') {
        if (result.parameters === null || result.parameters === undefined) {
          ...
          extractionResult = await this.stagehandPage.page.extract(result.parameters);
        }
      }
  ...
  private async getSummary(goal: string): Promise<string> {
    const { data: response } = (await this.llmClient.createChatCompletion<OperatorSummary>({
      options: {
        messages: [
          ...this.messages,
          {
  ...
  private async executeAction(
    ...
    switch (method) {
      ...
      case 'refresh':
        ...
        break;
      default:
        throw new StagehandError(`Error in OperatorHandler: Cannot execute unknown action: ${method}`);
    }
  }
}
